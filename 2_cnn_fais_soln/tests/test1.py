...
from app.config import SHOE_IMAGES_FOLDER, TEST_SET_MODIFY_FOLDER
...
from app.search import load_index
from app.config import FAISS_INDEX_PATH, CLIP_FAISS_INDEX_PATH
...
# Initialize services
cnn_faiss_service = CNNFaissSearch(cnn_index, extract_embedding, None)  # Replace None with actual search fn if needed
clip_faiss_service = CLIPFaissSearch(clip_index, extract_clip_embedding, None)  # Replace None with actual search fn if needed

cnn_search_controller = SearchController(cnn_faiss_service)
clip_search_controller = SearchController(clip_faiss_service)

# Ask user for test set folder name
test_set_folder_name = input("Enter test set folder name to save modified images and logs: ").strip()
modified_images_folder = os.path.join(TEST_SET_MODIFY_FOLDER, test_set_folder_name)
...
async def run_search_test(search_controller, metadata_col, log_file_path, method_name):
    ...
    for idx, img_meta in enumerate(main_images, 1):
        ...
        params = {}  # Replace with your SearchRequest or params if needed
        ...
        # apply_nothing(image_abs_path, modified_image_path)
...
async def main():
    ...
    await run_search_test(
        search_controller=cnn_search_controller,
        metadata_col=embedding_cnn_faiss_metadata_col,
        log_file_path=os.path.join(modified_images_folder, f"{test_set_folder_name}_cnn.log"),
        method_name="CNN"
    )

    # Run CLIP test
    await run_search_test(
        search_controller=clip_search_controller,
        metadata_col=embedding_clip_faiss_metadata_col,
        log_file_path=os.path.join(modified_images_folder, f"{test_set_folder_name}_clip.log"),
        method_name="CLIP"
    )
